...
'{station_to_analyze_perc}' aún no ha sido implementada.")
pass
...
"""Genera el contenido HTML estándar del popup de una estación, opcionalmente incluyendo el
mini-gráfico."""
station_name = row[Config.STATION_NAME_COL]
# 1. Obtener estadísticas de disponibilidad
df_station_data = df_anual_melted[df_anual_melted[Config.STATION_NAME_COL] ==
station_name]
...
valid_years = summary_data.get('años_validos', 0)
...
df_monthly_avg = \
df_station_monthly_avg.groupby(Config.MONTH_COL)[Config.PRECIPITATION_COL].mean().reset_i
ndex()
...
# CORRECCIÓN DE SINTAXIS/LÓGICA: Se usan trazas ocultas para mostrar los colores en la
leyenda
...
def get_map_options():
return {
"CartoDB Positron (Predeterminado)": {"tiles": "cartodbpositron", "attr": '&copy; <a
href="https://carto.com/attributions">CartoDB</a>', "overlay": False},
"OpenStreetMap": {"tiles": "OpenStreetMap", "attr": '&copy; <a
href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors', "overlay":
False},
"Topografía (OpenTopoMap)": {"tiles": "https://{s}.tile.opentomap.org/{z}/{x}/{y}.png", "attr":
'Map data: &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>
contributors, <a href="http://viewfinderpanoramas.org">SRTM</a> | Map style: &copy; <a
href="https://opentopomap.org">Open Topo Map</a> (<a
href="https://creativecommons.org/licenses/by-sa/3.0/">CC-BY-SA</a>)', "overlay": False},
"Relieve (Stamen Terrain)": {"tiles": "Stamen Terrain", "attr": 'Map tiles by <a
href="http://stamen.com">Stamen Design</a>, <a
href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a
href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors', "overlay":
False},
"Relieve y Océanos (GEBCO)": {"url":
"https://www.gebco.net/data_and_products/gebco_web_services/web_map_service/web_map_s
ervice.php", "layers": "GEBCO_2021_Surface", "transparent": False, "attr": "GEBCO 2021",
"overlay": True},
"Mapa de Colombia (WMS IDEAM)": {"url":
"https://geoservicios.ideam.gov.co/geoserver/ideam/wms", "layers": "ideam:col_admin",
"transparent": True, "attr": "IDEAM", "overlay": True},
"Cobertura de la Tierra (WMS IGAC)": {"url":
"https://servicios.igac.gov.co/server/services/IDEAM/IDEAM_Cobertura_Corine/MapServer/WMSS
erver", "layers": "IDEAM_Cobertura_Corine_Web", "transparent": True, "attr": "IGAC", "overlay":
True},
}
...
sub_tab_mapa, sub_tab_grafico = st.tabs(["Mapa Interactivo", "Gráfico de Disponibilidad de
Datos"])
with sub_tab_mapa:
...
summary_text = f"**Período:** {st.session_state.year_range[0]} -
{st.session_state.year_range[1]}\n\n"
if 'min_data_perc_slider' in st.session_state:
summary_text += f"**% Mínimo de Datos:**
{st.session_state.min_data_perc_slider}%\n\n"
...
if 'municipios_multiselect' in st.session_state and
st.session_state.municipios_multiselect:
summary_text += f"**Municipio:** {',
'.join(st.session_state.municipios_multiselect)}\n\n"
...
st.info("Mostrando la composición de datos originales vs. completados para el período
seleccionado.")
...
data_composition['total']) * 100
sort_order_comp = st.radio("Ordenar por:", ["% Datos Originales (Mayor a Menor)", "%
Datos Originales (Menor a Mayor)", "Alfabético"], horizontal=True, key="sort_comp")
...
fig_comp = px.bar(df_plot, x=Config.STATION_NAME_COL, y='Porcentaje', color='Tipo
de Dato',
...
st.info("Mostrando el porcentaje de disponibilidad de datos según el archivo de
estaciones.")
sort_order_disp = st.radio("Ordenar estaciones por:", ["% Datos (Mayor a Menor)", "%
Datos (Menor a Mayor)", "Alfabético"], horizontal=True, key="sort_disp")
df_chart = gdf_display.copy()
if "% Datos (Mayor a Menor)" in sort_order_disp: df_chart =
df_chart.sort_values(Config.PERCENTAGE_COL, ascending=False)
elif "% Datos (Menor a Mayor" in sort_order_disp: df_chart =
df_chart.sort_values(Config.PERCENTAGE_COL, ascending=True)
...
st.info(f"Mostrando análisis para {selected_stations_str} en el período
{st.session_state.year_range[0]} - {st.session_state.year_range[1]} y meses seleccionados.")
...
chart_anual = \
alt.Chart(df_anual_melted.dropna(subset=[Config.PRECIPITATION_COL])).mark_line(point=True).e
ncode(
...
).properties(title=f'Precipitación Anual por Estación ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})').interactive()
...
st.caption(f"Período de análisis: {st.session_state.year_range[0]} -
{st.session_state.year_range[1]}")
chart_type_annual = st.radio("Seleccionar tipo de gráfico:", ("Gráfico de Barras
(Promedio)", "Gráfico de Cajas (Distribución)"), key="avg_chart_type_annual", horizontal=True)
...
sort_order = st.radio("Ordenar estaciones por:", ["Promedio (Mayor a Menor)",
"Promedio (Menor a Mayor)", "Alfabético"], horizontal=True, key="sort_annual_avg")
if "Mayor a Menor" in sort_order: df_summary =
df_summary.sort_values(Config.PRECIPITATION_COL, ascending=False)
elif "Menor a Mayor" in sort_order: df_summary =
df_summary.sort_values(Config.PRECIPITATION_COL, ascending=True)
else: df_summary = df_summary.sort_values(Config.STATION_NAME_COL,
ascending=True)
fig_avg = px.bar(df_summary, x=Config.STATION_NAME_COL,
y=Config.PRECIPITATION_COL,
title=f'Promedio de Precipitación Anual por Estación
({st.session_state.year_range[0]} - {st.session_state.year_range[1]})',
...
mensual_graf_tab, mensual_enso_tab, mensual_datos_tab = st.tabs(["Gráfico de Serie
Mensual", "Análisis ENSO en el Período", "Tabla de Datos"])
with mensual_graf_tab:
if not df_monthly_filtered.empty:
control_col1, control_col2 = st.columns(2)
chart_type = control_col1.radio("Tipo de Gráfico:", ["Líneas y Puntos", "Nube de Puntos",
"Gráfico de Cajas (Distribución Mensual)"], key="monthly_chart_type")
color_by_disabled = (chart_type == "Gráfico de Cajas (Distribución Mensual)")
color_by = control_col2.radio("Colorear por:", ["Estación", "Mes"],
...
st.altair_chart(final_chart.properties(title=f"Serie de Precipitación Mensual
({st.session_state.year_range[0]} - {st.session_state.year_range[1]})").interactive(),
use_container_width=True)
else: # Gráfico de Cajas (Distribución Mensual)
st.subheader("Distribución de la Precipitación Mensual")
fig_box_monthly = px.box(df_monthly_filtered, x=Config.MONTH_COL,
y=Config.PRECIPITATION_COL,
color=Config.STATION_NAME_COL, title='Distribución de la Precipitación
por Mes',
labels={Config.MONTH_COL: 'Mes', Config.PRECIPITATION_COL:
'Precipitación Mensual (mm)', Config.STATION_NAME_COL: 'Estación'})
...
enso_filtered = \
st.session_state.df_enso[(st.session_state.df_enso[Config.DATE_COL].dt.year >=
st.session_state.year_range[0]) &
(st.session_state.df_enso[Config.DATE_COL].dt.year <=
st.session_state.year_range[1]) &
...
labels={Config.MONTH_COL: 'Mes', Config.PRECIPITATION_COL: 'Precipitación Promedio
(mm)'},
...
fig_avg_monthly.update_layout(height=600, xaxis=dict(tickmode='array',
...
title=f'Distribución Anual de Precipitación ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})',
...
color=Config.STATION_NAME_COL,
title=f'Distribución Mensual de Precipitación ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})',
...
color=Config.STATION_NAME_COL,
title=f'Precipitación Acumulada por Año ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})',
labels={Config.YEAR_COL: 'Año', Config.PRECIPITATION_COL: 'Precipitación Acumulada
(mm)'})
...
df_relacion = \
df_anual_melted.groupby(Config.STATION_NAME_COL)[Config.PRECIPITATION_COL].mean().reset_
index()
...
if not stations_for_analysis:
st.warning("Seleccione una o más estaciones en el panel lateral para calcular la serie
regional.")
elif df_monthly_filtered.empty:
st.warning("No hay datos mensuales para las estaciones seleccionadas para calcular la serie
regional.")
else:
...
st.info("Demasiadas estaciones seleccionadas para superponer (>10). Mostrando solo el
promedio regional.")
fig_regional.add_trace(go.Scatter(
...
xaxis_title="Fecha", yaxis_title="Precipitación Mensual (mm)", height=650
)
st.plotly_chart(fig_regional, use_container_width=True)
st.markdown("#### Datos de la Serie Regional Promedio")
df_regional_avg.rename(columns={'Precipitación Promedio': 'Precipitación Promedio
Regional (mm)'}, inplace=True)
...
{st.session_state.year_range[0]} - {st.session_state.year_range[1]}.")
tab_names = ["Animación GIF (Antioquia)", "Mapa Interactivo de Estaciones", "Visualización
Temporal",
"Gráfico de Carrera", "Mapa Animado", "Comparación de Mapas", "Interpolación
Comparativa"]
...
style="width:70%;" key={st.session_state["gif_reload_key"]}>',
...
# Si no hay datos mensuales para el gráfico, al menos colocar el marcador con
información básica.
else:
popup_html = generate_station_popup_html(station_data, df_anual_melted) #
Popup básico
...
st.warning(f"No se encontró información geográfica para la estación
{station_to_show}.")
with temporal_tab:
...
st.metric("Promedio Anual", f"{df_year_filtered[Config.PRECIPITATION_COL].mean():.0f}
mm")
st.metric("Máximo Anual", f"{df_year_filtered[Config.PRECIPITATION_COL].max():.0f}
mm")
with map_col:
...
st.warning("No hay datos de estaciones válidos para el año seleccionado en el
mapa.")
else:
...
labels={Config.PRECIPITATION_COL: 'Precipitación Anual (mm)',
Config.STATION_NAME_COL: 'Estación'},
title=f"Evolución de Precipitación Anual por Estación ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})"
)
...
st.info("Este mapa utiliza Plotly. Los controles de mapa se encuentran en las otras pestañas de
mapas.")
if not df_anual_melted.empty:
...
f"<b>Estación:</b> {row[Config.STATION_NAME_COL]}<br><b>Precipitación:</b>
{row[Config.PRECIPITATION_COL]:.0f} mm"
if pd.notna(row[Config.PRECIPITATION_COL]) else f"<b>Estación:</b>
{row[Config.STATION_NAME_COL]}<br><b>Precipitación:</b> Sin datos",
axis=1)
...
# CORRECCIÓN FINAL PLOTLY: ELIMINAR COLUMNA GEOMETRY ANTES DE PLOTLY (para
evitar TypeError)
...
title=f'Precipitación Anual por Estación ({st.session_state.year_range[0]} -
{st.session_state.year_range[1]})',
...
if not stations_for_analysis:
st.warning("Por favor, seleccione al menos una estación para ver esta sección.")
elif not df_anual_melted_non_na.empty and \
len(df_anual_melted_non_na[Config.YEAR_COL].unique()) > 0:
...
# Aseguramos la columna de geometría para Folium/GeoPandas y las coordenadas
numéricas
...
fig = go.Figure()
fig.update_layout(title=f"Datos insuficientes para {method} en {year} (se necesitan >=
4)",
xaxis_visible=False, yaxis_visible=False)
return fig
# Usamos las columnas numéricas LAT/LON del DataFrame mergeado para la
interpolación
...
**análisis de percentiles** para extremos puntuales y el **Índice Estandarizado de Precipitación
(SPI)** para evaluar la intensidad de la sequía o humedad.")
if not stations_for_analysis:
st.warning("Por favor, seleccione al menos una estación para ver esta sección.")
return
percentile_sub_tab, spi_sub_tab = st.tabs(["Análisis por Percentiles", "Análisis SPI"])
with percentile_sub_tab:
station_to_analyze_perc = st.selectbox("Seleccione una estación para el análisis de
percentiles:",
...
help="Una escala corta (3 meses) refleja sequías agrícolas. Una escala larga
(12-24 meses) refleja sequías hidrológicas.")
if station_to_analyze_spi:
df_station_spi = df_monthly_filtered[df_monthly_filtered[Config.STATION_NAME_COL] ==
station_to_analyze_spi].copy()
...
with col2_spi:
st.warning(f"No hay suficientes datos ({len(precip_series.dropna())} meses) para
calcular el SPI-{spi_window}.")
else:
...
(df_plot['spi'] > -1.5) & (df_plot['spi'] <= -1.0), (df_plot['spi'] > -1.0) & (df_plot['spi'] <
1.0),
(df_plot['spi'] >= 1.0) & (df_plot['spi'] < 1.5), (df_plot['spi'] >= 1.5) & (df_plot['spi'] <
2.0),
df_plot['spi'] >= 2.0
]
...
fig = go.Figure()
fig.add_trace(go.Bar(x=df_plot.index, y=df_plot['spi'], marker_color=df_plot['color'],
name='SPI'))
fig.update_layout(title=f"Índice Estandarizado de Precipitación (SPI-{spi_window}) para
{station_to_analyze_spi}",
...
st.warning("No se puede realizar el análisis de anomalías. El DataFrame base no está
disponible.")
return
df_long_filtered_stations = \
...
return
df_climatology = df_long_filtered_stations.groupby([Config.STATION_NAME_COL,
Config.MONTH_COL])[Config.PRECIPITATION_COL].mean().reset_index().rename(columns={Config.
PRECIPITATION_COL: 'precip_promedio_mes'})
df_anomalias = pd.merge(df_monthly_filtered, df_climatology, on=[Config.STATION_NAME_COL,
Config.MONTH_COL], how='left')
df_anomalias['anomalia'] = df_anomalias[Config.PRECIPITATION_COL] -
df_anomalias['precip_promedio_mes']
...
st.warning("No hay suficientes datos históricos para calcular y mostrar las anomalías con los
filtros actuales.")
return
...
'Anomalía (mm)', Config.PRECIPITATION_COL: 'Ppt. (mm)', 'precip_promedio_mes': 'Ppt. Media
(mm)'}).round(0), use_container_width=True)
with col2:
...
st.info(f"Mostrando análisis para {selected_stations_str} en el período
{st.session_state.year_range[0]} - {st.session_state.year_range[1]}.")
matriz_tab, resumen_mensual_tab, sintesis_tab = st.tabs(["Matriz de Disponibilidad", "Resumen
Mensual", "Síntesis General"])
with matriz_tab:
...
{int(max_annual_row[Config.YEAR_COL])})"
)
with col2:
st.metric(
...
({meses_map.get(max_monthly_row[Config.MONTH_COL])}
{max_monthly_row[Config.DATE_COL].year})"
)
else:
...
variables (otras estaciones o índices climáticos) utilizando el coeficiente de correlación de
Pearson.")
if not stations_for_analysis:
st.warning("Por favor, seleccione al menos una estación para ver esta sección.")
return
enso_corr_tab, station_corr_tab, indices_climaticos_tab = st.tabs(["Correlación con ENSO
(ONI)", "Comparación entre Estaciones", "Correlación con Otros Índices"])
with enso_corr_tab:
...
st.warning("No hay datos coincidentes entre la precipitación y el ENSO para la selección
actual.")
return
analysis_level = st.radio("Nivel de Análisis de Correlación con ENSO", ["Promedio de la
selección", "Por Estación Individual"], horizontal=True, key="enso_corr_level")
...
if p_value < 0.05:
st.success("La correlación es estadísticamente significativa, lo que sugiere una relación
lineal entre las variables.")
else:
st.warning("La correlación no es estadísticamente significativa. No hay evidencia de una
...
st.warning("No hay suficientes datos superpuestos para calcular la correlación para las
...
st.warning("No se encontraron columnas para los índices climáticos (SOI o IOD) en el archivo
principal o no hay datos en el período seleccionado.")
else:
...
# CORRECCIÓN DE LÓGICA: Mapear la etiqueta seleccionada al nombre de columna en
Config
...
st.error(f"La columna para el índice '{selected_index}' no se encontró en los datos de la
estación.")
...
{selected_station_corr})")
st.metric("Coeficiente de Correlación (r)", f"{corr:.3f}")
if p_value < 0.05:
st.success("La correlación es estadísticamente significativa, lo que sugiere una
...
st.warning("No hay suficientes datos superpuestos entre la estación y el índice para
...
st.warning("No se encontraron datos del fenómeno ENSO en el archivo de precipitación
cargado.")
return
...
var_name][0]
df_enso_filtered = df_enso[(df_enso[Config.DATE_COL].dt.year >=
st.session_state.year_range[0]) &
(df_enso[Config.DATE_COL].dt.year <= st.session_state.year_range[1]) &
(df_enso[Config.DATE_COL].dt.month.isin(st.session_state.meses_numeros))]
if not df_enso_filtered.empty and var_code in df_enso_filtered.columns and not \
df_enso_filtered[var_code].isnull().all():
fig_enso_series = px.line(df_enso_filtered, x=Config.DATE_COL, y=var_code,
...
enso_anim_data_filtered = enso_anim_data[(enso_anim_data[Config.DATE_COL].dt.year >=
st.session_state.year_range[0]) &
(enso_anim_data[Config.DATE_COL].dt.year <=
st.session_state.year_range[1])]
selected_date = None
with controls_col:
st.markdown("##### Controles de Mapa")
...
def display_trends_and_forecast_tab(df_anual_melted, df_monthly_to_process,
