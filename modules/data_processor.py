...
continue
st.error(f"No se pudo decodificar el archivo '{file_uploader_object.name}' con las codificaciones
probadas.")
return None
@st.cache_data
...
date_range = pd.date_range(start=df_station.index.min(), end=df_station.index.max(),
freq='MS')
...
progress_bar.progress((i + 1) / len(station_list), text=f"Completando series... Estación:
{station}")
progress_bar.empty()
return pd.concat(all_completed_dfs, ignore_index=True)
@st.cache_data
def load_and_process_all_data(uploaded_file_mapa, uploaded_file_precip,
...
lon_col = next((col for col in df_stations_raw.columns if 'longitud' in col.lower() or 'lon' in
col.lower()), None)
lat_col = next((col for col in df_stations_raw.columns if 'latitud' in col.lower() or 'lat' in
col.lower()), None)
...
df_stations_raw[lon_col] = pd.to_numeric(df_stations_raw[lon_col].astype(str).str.replace(',', '.',
regex=False), errors='coerce')
df_stations_raw[lat_col] = pd.to_numeric(df_stations_raw[lat_col].astype(str).str.replace(',', '.',
regex=False), errors='coerce')
...
pd.to_numeric(gdf_stations[Config.ALTITUDE_COL].astype(str).str.replace(',', '.',
regex=False), errors='coerce')
...
df_long[col] = pd.to_numeric(df_long[col].astype(str).str.replace(',', '.', regex=False),
errors='coerce')
...
df_enso[col] = pd.to_numeric(df_enso[col].astype(str).str.replace(',', '.', regex=False),
errors='coerce')
